...
export interface TeamScheduler {
  ...
  /** Stop queued or newly observed work before the team is archived. */
  closeTeam(workspace: string, teamId: string): void
  /** Re-enable a newly created team whose id was previously archived. */
  activateTeam(workspace: string, teamId: string): void
}
...
function schedulerTeamKey(stateRoot: string, teamId: string): string {
  return [stateRoot, teamId].join('\u0000')
}
...
export function installTeamScheduler(ctx: Context, config: SchedulerConfig): TeamScheduler {
  ...
  const closingTeams = new Set<string>()
  ...
  const runtime: TeamScheduler = {
    closeTeam(workspace, teamId) {
      closingTeams.add(schedulerTeamKey(stateRootOf(workspace, config), teamId))
    },

    activateTeam(workspace, teamId) {
      closingTeams.delete(schedulerTeamKey(stateRootOf(workspace, config), teamId))
    },

    async kickTeam(workspace, teamId, suppliedCaptain) {
      const stateRoot = stateRootOf(workspace, config)
      if (closingTeams.has(schedulerTeamKey(stateRoot, teamId))) return
    ...
    async kickMember(workspace, teamId, memberName, suppliedCaptain) {
      const stateRoot = stateRootOf(workspace, config)
      const teamKey = schedulerTeamKey(stateRoot, teamId)
      const queueKey = [teamKey, memberName].join('\u0000')
      await serializeMember(queueKey, async () => {
        if (closingTeams.has(teamKey)) return
        ...
        const ticket = await withTeamLock(teamLockKey(stateRoot, team.id), async (): Promise<DispatchTicket | undefined> => {
          if (closingTeams.has(teamKey)) return undefined
  ...
  const syncMemberStatus = async (agent: Agent, status: AgentStatus): Promise<void> => {
    ...
    const teamKey = schedulerTeamKey(stateRoot, located.id)
    if (closingTeams.has(teamKey)) return
    ...
    if (status === 'idle' && !closingTeams.has(teamKey)) {
