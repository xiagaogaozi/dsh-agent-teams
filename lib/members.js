 ...
 */
function customPersonaProtocol(team, member, stateDir) {
...
export async function spawnMember(ctx, config, selections, llmSelection, captain, team, member, stateDir, signal) {
    ...
    if (child === undefined) {
        throw new Error(`agent-teams: preset "${presetId}" for member "${member.name}" requires an in-process subagent provider`);
    }
    try {
