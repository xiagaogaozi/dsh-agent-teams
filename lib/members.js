 ...
 */
function teamPlumbing(team, member, stateDir) {
...
export async function spawnMember(ctx, config, captain, team, member, stateDir, signal) {
    ...
    if (presetId !== undefined) {
        if (child === undefined) {
            throw new Error(`agent-teams: preset "${presetId}" for member "${member.name}" requires an in-process `
    ...
    if (member.reasoningEffort !== undefined && member.reasoningEffort !== '') {
        if (child === undefined) {
            throw new Error(`agent-teams: reasoningEffort for member "${member.name}" requires an in-process `
                + `subagent provider (the child is not registered in the agents service)`);
        }
 ...
 */
async function supportsEffort(ctx, memberModel, configModel, captain, effort) {
