...
export function registerAgentTeamsTools(ctx, config) {
    ...
    ctx.tools.register(defineTool({
        ...
        parameters: {
            ...
            tool_allow: { type: 'array', items: { type: 'string' }, description: 'Optional white-list of tools visible to this member (white-list mode): only these tools plus the agent_teams_* collaboration tools remain. When unset, the member keeps its inherited/preset tool set minus the captain-only tools.' },
        },
        output: {
            schema: {
                type: 'object',
