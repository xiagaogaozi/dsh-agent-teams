...
function splitMemberModelSpec(spec) {
    if (spec === undefined)
        return {};
...
async function resolveRequestedEffort(ctx, provider, model, inheritedEffort, requestedEffort, signal) {
    if (requestedEffort === undefined || requestedEffort.trim() === '')
        return inheritedEffort;
    try {
        const resolved = await ctx.llm.resolveCallConfig({
            provider,
            model,
            reasoningEffort: ReasoningEffortId(requestedEffort),
        }, signal);
        return resolved.reasoningEffort === undefined ? inheritedEffort : String(resolved.reasoningEffort);
    }
    catch {
        return inheritedEffort;
    }
...
export function registerAgentTeamsTools(ctx, config) {
        ...
        async execute(args, exec) {
            ...
            return withTeamLock(captainLockKey(stateRoot, captain.id), async () => {
                ...
                return withTeamLock(teamLockKey(stateRoot, teamId), async () => {
                    ...
                    scheduler.activateTeam(workspace, state.id);
    ...
    ctx.tools.register(defineTool({
        ...
        parameters: {
            ...
            preset: { type: 'string', description: 'Optional member preset. The configured default is used when this is omitted.' },
            persona: { type: 'string', description: 'Optional member persona. AgentTeams task rules remain appended.' },
            reasoning_effort: { type: 'string', description: 'Optional reasoning level. Unsupported values fall back to the resolved model default.' },
            template: { type: 'string', description: 'Optional template name from the 团队 settings page. Explicit provider, model, preset, persona, and reasoning_effort win.' },
        },
        output: {
            schema: {
                type: 'object',
        ...
        async execute(args, exec) {
            ...
            const created = await withTeamLock(teamLockKey(stateRoot, team.id), async () => {
                ...
                const template = args.template === undefined
                    ? undefined
                    : config.loadProfiles().find(candidate => candidate.name === args.template);
                ...
                const templateRoute = splitMemberModelSpec(template?.model === '' ? undefined : template?.model);
                const requestedRoute = splitMemberModelSpec(args.model);
                const selection = await resolveMemberLlmSelection(ctx, captain, {
                    provider: args.provider ?? requestedRoute.provider ?? templateRoute.provider,
                ...
                const reasoningEffort = await resolveRequestedEffort(ctx, selection.provider, selection.model, selection.reasoningEffort, requestedEffort, exec.signal);
                ...
                const member = {
                    ...
                    preset: args.preset ?? (template?.preset === '' ? undefined : template?.preset),
                    persona: args.persona,
                    joinedAt: Date.now(),
                    status: 'idle',
                };
                await spawnMember(ctx, memberRuntime(config), memberSelections, resolvedSelection, captain, fresh, member, config.stateDir, exec.signal);
                ...
                return {
                    ...
                    provider: resolvedSelection.provider,
                    model: resolvedSelection.model,
                    ...resolvedSelection.reasoningEffort === undefined
                        ? {}
                        : { reasoning_effort: resolvedSelection.reasoningEffort },
    ...
    ctx.tools.register(defineTool({
        ...
        async execute(_args, exec) {
            ...
            const stateRoot = stateRootOf(workspace, config);
            const team = await requireCaptainTeam(workspace, config, captain);
            scheduler.closeTeam(workspace, team.id);
