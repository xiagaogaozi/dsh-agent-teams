...
function Selector({ value, options, display, emptyLabel, onSelect, disabled, }) {
    ...
    return (_jsx(Menu, { open: open, onClose: () => { setOpen(false); }, items: items, selectedId: value, onSelect: (id) => { onSelect(id); setOpen(false); }, align: "end", portal: true, anchor: (_jsxs("button", { type: "button", className: css.selector, "aria-haspopup": "menu", "aria-expanded": open, disabled: disabled, onClick: () => { setOpen(v => !v); }, children: [_jsx("span", { className: css.selectorText, children: value === '' ? emptyLabel : display(value) }), _jsx(IconChevronDownOutline14, { className: css.chevron })] })) }));
}
